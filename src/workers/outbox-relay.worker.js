...
export async function startOutboxRelayWorker() {
  ...
  const stopWorker = async () => {
    ...
    if (isConnecting && connectPromise) {
      try {
        ...
        console.error(
          "[OutboxRelay] Connection failed during shutdown cleanup:",
          err.message,
        );
      }
    }
  ...
  console.log(
    `[OutboxRelay] Polling every ${POLL_INTERVAL}ms, batch size: ${BATCH_SIZE}`,
  );
  ...
  const scheduleNextPoll = () => {
    ...
    timerId = setTimeout(async () => {
      activePollPromise = pollAndRelay().catch((err) => {
        console.error(
          `[OutboxRelay] Unhandled Error during poll: `,
          err.message,
        );
      });
